...
class CasoController extends Controller
{
    ...
    public function getStatics(Request $request)
    {
        ...
        $casos_psicologo = $this->statisticByPsicologo($fechaInicio, $fechaFin);
        $casos_municipios = $this->statisticByMunicipios($fechaInicio, $fechaFin);
        $casos_origen = $this->statisticByOrigen($fechaInicio, $fechaFin, $totalCasos);
    ...
    private function statisticByLineaIntervencion($fechaInicio, $fechaFin)
    {
        ...
        $casos_zona->add(
            json_decode('{"nombre": "Anonimo", "total": ' . $casos_zona_anonimos . '}')
        );
    ...
    {
        $casos_zona = Caso::selectRaw('motivo_consultas.name as nombre, count(motivo_consulta_id) as total')
    ...
    private function statisticByGenero($fechaInicio, $fechaFin)
    {
        ...
        $casos_genero->add(
            json_decode('{"nombre": "Anonimo", "total": ' . $casos_genero_anonimos . '}')
        );
    ...
    private function statisticBySexo($fechaInicio, $fechaFin)
    {
        ...
        $casos_sexo_anonimos = Caso::query()
            ...
            ->count();
        $casos_sexo->add(
            json_decode('{"nombre": "Anonimo", "total": ' . $casos_sexo_anonimos . '}')
        );
        return $casos_sexo;
    }
    ...
    private function statisticByMunicipios($fechaInicio, $fechaFin)
    {
        ...
        $casos_anonimos = Caso::selectRaw('municipios.name as nombre, count(municipio_id) as total')
            ->join('pacientes', 'casos.paciente_id', '=', 'pacientes.id')
            ->byAuthUser()
            ->filterByDate($fechaInicio, $fechaFin)
        ...
        $statistic->nombre = 'Anonimo';
        $statistic->total = $casos_anonimos;
        $casos_municipios->push($statistic);
        return $casos_municipios;
    }


    private function statisticByOrigen($fechaInicio, $fechaFin, &$totalCasos)
    {
        ...
        return $casos_origen->map(function ($caso) use (&$totalCasos) {
            $statistic = new \stdClass();
    ...
    private function statisticByPsicologo($fechaInicio, $fechaFin)
    {
    ...
    {
        return trim(
            $this->getString($usuario->name) . ' ' .
                $this->getString($usuario->last_name)
        );
    }
